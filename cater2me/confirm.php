...
$_GET['oid']=(int)$_GET['oid'];
	...
	//***************
	$res = mysql_query('SELECT 1 FROM tmp_vendor_confirmations WHERE order_id = '.$_GET['oid']) or die(mysql_error());
	...
	mysql_query('REPLACE INTO tmp_vendor_confirmations (order_id, date) Values ("'.$_GET['oid'].'","'.$datetimeday.'")' );

	
	
sendMail($config['staff_notifications']['vendor_confirmations'],'Order confirmed by vendor','Order ID:'.$_GET['oid']."\nClient: ".$order['cname']."\nVendor: ".$order['vname']."\nWhen: ".$order['order_for']);
...
if(isset($_GET['ajax'])) die('ok');
		...
		notif('Thank you for confirming this order. All orders are updated every 24 hours.');
		/*** temporary ***/


$template = array(
	'title' => 'Cater2.me | Order confirmation',
	'menu_selected' => 'home',
	'header_resources' => '
	...
	',
	'breadcrumb' => array('Home'=>'/','Dashboard'=>'/dashboard/','Order conformation (#'.$_GET['oid'].')'=>$_SERVER['REQUEST_URI']),
	
	'grey_bar' => 'Order confirmation #'.$_GET['oid'],
	);
...
?>
<h1>Thank you. This order has been confirmed.</h1>
