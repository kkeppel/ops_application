...
add_action( 'widgets_init', 'jg_recent_posts' );

// Register widget
function jg_recent_posts() {
	register_widget( 'jg_recent_posts' );
}

// Widget class
class jg_recent_posts extends WP_Widget {
...
function jg_recent_posts() {
	...
	// Widget settings
	$widget_ops = array(
		'classname' => 'jg_recent_posts',
		'description' => __('Displays your most recent posts.', 'Bowtie')
	);

	/* Create the widget */
	$this->WP_Widget( 'jg_recent_posts', __('Bowtie Recent Posts', 'Bowtie'), $widget_ops );
}
