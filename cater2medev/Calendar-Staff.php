...
/**** to fix */
	$hash =((((( $order['id_order'] + 5)*152) -2)/2));
			...
			echo '
			<h3><a href="#order'.$order['id_order'].'">['.$order['name'].'] '.date('g:i A', strtotime($order['order_for'])).' - '.$address['address'].' [Order #'.$order['id_order_proposal'].']'.$status.'</a>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<form target="_blank"><input type="button" value="Print Order" OnClick="window.open(\'/order-pdf.php?a='.$hash.'\')"></form>
			<form target="_blank"><input type="button" value="Print Labels" OnClick="window.open(\'/label-pdf.php?a='.$hash.'\')"></form>
			<form target="_blank"><input type="button" value="Print Menu Day" OnClick="window.open(\'/menu-pdf.php?a='.$hash.'\')"></form>
			</h3>
			<div>';
		
			/*
echo  '<a href="/order-pdf.php?a='.$order['id_order'].'" target="_blank">Print Order PDF</a><br>';
			echo  '<a href="/label-pdf.php?a='.$order['id_order'].'"target="_blank">Print Labels PDF</a><br>';
			echo  '<a href="/menu-pdf.php?a='.$order['id_order'].'"target="_blank">Print Menu Day PDF</a>';
*/
