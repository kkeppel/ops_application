	...
	<div class="container_12" id="body-wrap" role="main">
        ...
        <?php $query = new WP_Query(); 
			  $query->query('post_type=portfolio&posts_per_page=-1'); ?>
                ...
                <!--BEGIN #columns-wrap-->
            	<ul id="portfolio" style="margin-top:0px;">
				...
				$terms = get_the_terms( get_the_ID(), 'skill-type' );  ?> 
					...
					<li data-id="id-<?php echo $count; ?>" class="<?php foreach ($terms as $term) { echo strtolower(preg_replace('/\s+/', '-', $term->name)). ' '; } ?> portfolio_two_column grid_6 alpha" style="margin-top:20px;">
					...
					<?php if (has_post_thumbnail()) { /* if the post has a featured image */ 
						...
						$image = vt_resize( $thumb,'' , 454, 194, true ); // Resize original image
					?>
					...
					<?php } elseif (!has_post_thumbnail()) { // if there is no thumbnail, show a generic image holder ?>
						...
						<a href="<?php the_permalink() ?>">
							<img src="<?php bloginfo('template_directory'); ?>/images/no-image-454x194.jpg" title="<?php the_title() ?>"  />
						</a>
				...
				<div class="portfolio-details">
                    ...
                    <div class="portfolio-excerpt">
						...
						<?php the_excerpt(); ?>
                        <a href="<?php the_permalink(); ?>" class="more-link"><?php _e('read more &rarr;', 'bowtie'); ?></a>
					...
					</div> <!-- end .grid_2 Portfolio Meta Details -->
