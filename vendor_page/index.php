...
<head>
	...
	<link href="/vendor_page/css/splash.css" rel="stylesheet" category="text/css" />
		

    <link rel="icon" type="/vendor_page/image/vnd.microsoft.icon" href="vendor_page//favicon.ico" />
	<link rel="SHORTCUT ICON" href="/favicon.ico" />
	<script src="/vendor_page/js/jquery-1.3.2.min.js" type="text/javascript"></script>
	...
	<script>
	    // Load theme
	    Galleria.loadTheme('vendor_page/js/themes/lightbox/galleria.lightbox.js');
	</script>
	<?php } ?>
	...
	<script src="/vendor_page/js/jquery.prettyPhoto.js" type="text/javascript" charset="utf-8"></script>
	<link rel="stylesheet" href="/vendor_page/css/prettyPhoto.css" type="text/css" media="screen" title="prettyPhoto main stylesheet" charset="utf-8" />
	<?php } ?>

	<script type="text/javascript" src="/vendor_page/js/switches.js"></script>
...
-->
	...
	<div id="about" class="content_bubble_<? echo $position ?>">
		<h3>Bio</h3><br>
			...
			<div id="img_logo">
			<center><img width="300" src="/vendor_page/upload/<? echo $vendor_name?>_logo.jpg">	</center></div>
  ...
  ?></p>
	...
	<div id="comments" class="content_bubble_<? echo $position ?>">
		...
		<p>
		 ...
		 <form method="post" action="/vendor_page/index.php?a=body_comment/<? echo $vendor_name; ?>" name="add_com">
	 ...
	 <textarea rows="2" cols="60" name="text" class="textareacom" ></textarea>
		...
		<?php $i=0; $name = $_POST['name']; if ($name=='' ) {$name_c='anonymous';} else {$name_c=$name; } $text_c = $_POST['text']; $date = date("Y-m-d"); $id_v=$id; if (text_c != '' ) { $sql = "INSERT INTO Comments(id_c, text_c, date_c, name_c, is_flag_c, like_c, is_show_c, id_vendor_c) VALUES('','$text_c','$date','$name_c','0','0','1','$id_v')"; mysql_query($sql) or die('SQL error !'.$sql.'<br>'.mysql_error()); } 
...
<div id="scrollingComments">
		...
		$name_s = $_POST['name_s']; 
		if ($name_s=='' ) 
		{$name_s='anonymous';} else {$name_s=$name_s; } 
		$text_s = $_POST['text_s']; 
		$date_s = date("Y-m-d"); 
		$id_s=$_POST['id_subcomment']; 
			if ($text_s != '') { 
			$sql = "INSERT INTO Subcomments(id_subc, text_subc, date_subc, name_subc, is_flag_s, id_subc_c) VALUES('','$text_s','$date_s','$name_s','0','$id_s')"; 
...
$name=$row[1]; $datetmp=$row[2]; 
$date =date("d F Y", strtotime($datetmp)); ?>
<p><form method="post" action="/vendor_page/index.php?a=body_comment/<? echo $vendor_name; ?>">
<?php echo $comments ; ?> "
<br><span> by <?php echo $name ; echo ', ' .$date; ?> </span>
<p id="place_comments">
<? if ($like=='0') { echo '0 person like this comment';}
